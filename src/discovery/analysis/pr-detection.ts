...
const SECURITY_CLEARANCE_PATTERNS: Array<[string, RegExp]> = [
...
export function detectPrRequirements(jobText: string): PrRequirementResult {
  if (!jobText) {
    return {
      isPrRequired: false,
  ...
  let earliestClearanceMatch: [string, string, number] | null = null;
  ...
  if (earliestClearanceMatch) {
    ...
    matchedPatterns.push(
      `SECURITY_CLEARANCE_${earliestClearanceMatch[0]}: "${earliestClearanceMatch[1]}"`,
    );
  }
