 ...
 */
export function crossPlatformDedupKey(job: DiscoveryJob): string {
 ...
 */
export function jobRichness(job: DiscoveryJob): number {
  let score = 0;
  if (job.description) score += 2;
  if (job.salary || job.salaryRaw) score++;
  if (job.externalUrl) score++;
  if (job.jobType) score++;
  if (job.workArrangement) score++;
  if (job.companyLogoUrl) score++;
  if (job.requiredSkills) score++;
  if (job.techStack && job.techStack !== "[]") score++;
