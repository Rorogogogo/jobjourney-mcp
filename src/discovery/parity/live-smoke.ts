...
export async function runLiveParitySmoke(
  options: RunLiveParitySmokeOptions = {},
): Promise<LiveParitySmokeResult> {
  ...
  const executeTsRun =
    options.executeTsRun ??
    (async (runOptions: DiscoveryRunOptions) => {
  ...
  const executePythonRun =
    options.executePythonRun ?? ((liveRequest) => runPythonLiveSmoke(liveRequest));
...
export async function persistTsJobsToDatabase(
  jobs: DiscoveryJob[],
  request: LiveParityRequest,
): Promise<void> {
...
export function writeLiveParityReport(
  report: LiveParityReport,
  reportPath: string,
): string {
...
export function summarizeDiscoveryJobs(
  jobs: DiscoveryJob[],
  source: string,
): LiveParityEngineSummary {
  const filteredJobs = jobs.filter(
    (job) => job.source === source && isPrimaryPlatformJob(job, source),
  );
  ...
  return {
    ...
    atsBreakdown: filteredJobs.reduce<Record<string, number>>((acc, job) => {
...
function isPrimaryPlatformJob(job: DiscoveryJob, source: string): boolean {
...
async function runPythonLiveSmoke(
  request: LiveParityRequest,
): Promise<LiveParityEngineSummary> {
        ...
        reject(
          new Error(
            normalizeProcessError(
              stderr || stdout || `python live smoke exited with code ${code ?? "unknown"}`,
            ),
          ),
        );
        return;
  ...
  try {
    const totals = db
      ...
      .get() as {
      total_jobs: number;
      posted_at_count: number | null;
      external_url_count: number | null;
    };
    ...
    const atsRows = db
      ...
      .all() as Array<{ ats_type: string; job_count: number }>;
    ...
    return {
      ...
      atsBreakdown: Object.fromEntries(
        atsRows.map((row) => [row.ats_type, row.job_count]),
      ),
...
function createLiveParityRequest(
  options: RunLiveParitySmokeOptions,
): LiveParityRequest {
  return {
