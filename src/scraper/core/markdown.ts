...
export function renderMarkdownReport(jobs: ScrapedJob[]): string {
  if (jobs.length === 0) {
    return "# Job Results\n\nNo jobs found.";
  }
  ...
  const sections = jobs.map((job) => {
    ...
    if (job.workplaceType) lines.push(`- Workplace: ${job.workplaceType}`);
    if (job.postedDate) lines.push(`- Posted: ${job.postedDate}`);
    if (job.applicantCount) lines.push(`- Applicants: ${job.applicantCount}`);
    if (job.isAlreadyApplied) lines.push(`- Status: Already Applied`);
