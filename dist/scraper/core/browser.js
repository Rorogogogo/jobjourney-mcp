 ...
 */
export function createStopController(browser) {
    let _stopped = false;
 ...
 */
export async function injectScrapingOverlay(page, controller) {
    ...
    await page.evaluate(() => {
        if (document.getElementById("__jj-scrape-overlay"))
            return;
 ...
 */
export async function updateOverlayProgress(page, progress) {
    try {
        await page.evaluate((p) => {
            ...
            if (bar)
                bar.style.width = pct + "%";
            if (count)
                count.textContent = p.jobsCollected + " jobs collected";
        }, progress);
    }
    catch {
