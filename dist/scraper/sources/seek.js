import { launchBrowser, createAuthenticatedContext, saveCookies, createStopController, injectScrapingOverlay, updateOverlayProgress, isBrowserClosedError, } from "../core/browser.js";
...
async function scrapePageWithClickThrough(page, scrapeStart, controller, currentPage, totalPages, previousJobCount) {
    await page
    ...
    for (let i = 0; i < totalCards; i++) {
        ...
        try {
            const cardJob = await withTimeout(processCard(page, cards[i]), CARD_TIMEOUT_MS);
            if (cardJob) {
                jobs.push(cardJob);
            }
