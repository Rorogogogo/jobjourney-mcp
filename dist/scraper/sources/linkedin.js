...
export async function scrapeLinkedInPage(page, request) {
    ...
    const details = await page.evaluate(() => {
        ...
        for (const sel of descSelectors) {
            ...
            if (el) {
                ...
                description = inner.innerText?.replace(/\n{3,}/g, "\n\n").trim() ?? "";
                if (description)
                    break;
            }
        }
        ...
        let jobUrl = "";
        const titleLink = document.querySelector("h1.t-24 a, .job-details-jobs-unified-top-card__job-title h1 a");
        if (titleLink?.href)
...
async function extractBasicInfoFromCard(card) {
    const title = (await card.locator(".artdeco-entity-lockup__title, a.base-card__full-link, .base-search-card__title").first().textContent()) ?? "";
    const company = (await card.locator(".artdeco-entity-lockup__subtitle, .base-search-card__subtitle").first().textContent()) ?? "";
    const location = (await card.locator('span[class*="tvm__text"], .job-search-card__location').first().textContent()) ?? "";
...
function buildLinkedInUrl(keyword, location, start) {
