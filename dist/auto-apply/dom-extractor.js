function extractionLogic(skipVisibilityCheck) {
    const fields = [];
    const seen = new Set();
    ...
    function isVisible(el) {
        if (skipVisibilityCheck)
            return true;
        const htmlEl = el;
        if (!htmlEl.offsetParent && (htmlEl).style?.position !== "fixed")
            return false;
    ...
    function isHoneypot(el) {
        ...
        if (parseInt(style.left) < -1000 || parseInt(style.top) < -1000)
            return true;
        if (style.position === "absolute" && (style.left === "-9999px" || style.top === "-9999px"))
            return true;
        if (el.tabIndex === -1 && el.getAttribute("aria-hidden") === "true")
            return true;
        return false;
    }
    ...
    for (const el of elements) {
        if (!isVisible(el) || isHoneypot(el))
            continue;
        const uniqueSelector = getUniqueSelector(el);
        if (seen.has(uniqueSelector))
            continue;
        seen.add(uniqueSelector);
        const inputType = el.tagName === "SELECT"
            ? "select"
            : el.tagName === "TEXTAREA"
                ? "textarea"
                : (el.getAttribute("type") || "text").toLowerCase();
        if (["password", "search"].includes(inputType))
            continue;
        let options = [];
        ...
        if (inputType === "radio") {
            ...
            options = Array.from(radios).map((r) => {
                ...
                return lbl ? lbl.textContent?.trim() || r.value : r.value;
            });
        }
        fields.push({
 ...
 */
export function runExtractorInDom() {
 ...
 */
export function buildExtractorScript() {
...
export async function extractFormFields(page) {
    ...
    const mainResult = (await page.evaluate(script));
    const frames = page.frames();
    for (const frame of frames) {
        if (frame === page.mainFrame())
            continue;
        try {
            const frameResult = (await frame.evaluate(script));
