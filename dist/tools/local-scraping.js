...
export function registerLocalScrapingTools(server, deps = {}) {
    ...
    const discoveryLogger = (_payload) => {
    ...
    server.addTool({
        name: "discover_jobs",
        description: "Find jobs for the user by scraping live listings from job sites (LinkedIn, SEEK, Jora, etc.). This is the primary tool for job searching — always use this when the user wants to find, search, or look for jobs. Enriches results with ATS detection, expands career pages, stores results locally, then opens the web app for visualization.",
        ...
        parameters: z.object({
            ...
            career_discovery: z
                ...
                .optional()
                .default(false)
                .describe("When LinkedIn hides external apply URLs, probe company career pages to find ATS links (Greenhouse, Lever, etc.). Disabled by default because it probes many URLs and can cause timeouts in MCP clients."),
        }),
                ...
                const progressLogger = (payload) => {
                    ...
                    switch (event) {
                        ...
                        case "discovery_run_complete":
                            if (payload.crossPlatformDuplicatesRemoved > 0) {
                                sendProgress(`🔄 Removed ${payload.crossPlatformDuplicatesRemoved} cross-platform duplicates`);
                            }
                            break;
                    }
                };
                try {
                    ...
                    const result = await runDiscoveryImpl({
                        ...
                        onJobsBatch: (batchJobs) => {
                            ...
                            sendProgress(`💾 Saved ${persistedCount} jobs to database so far`);
                        },
                    });
    ...
    server.addTool({
        name: "list_schedules",
        description: "List all scheduled job discovery cron jobs, including disabled ones. Shows schedule ID, keyword, location, sources, cron expression, enabled status, and last run time.",
        parameters: z.object({}),
        execute: async () => {
            const db = openDatabaseImpl();
            try {
    ...
    server.addTool({
        ...
        description: "Enable or disable a scheduled job discovery cron job by its ID. The agent will pick up changes on its next reconcile cycle (within 60 seconds).",
        parameters: z.object({
    ...
    server.addTool({
        ...
        description: "Permanently delete a scheduled job discovery cron job by its ID. Use list_schedules to see available IDs.",
        parameters: z.object({
    ...
    server.addTool({
        ...
        description: "Permanently delete ALL scheduled job discovery cron jobs. Use with caution.",
        parameters: z.object({}),
        execute: async () => {
            const db = openDatabaseImpl();
            try {
    ...
    server.addTool({
        name: "login_jobsite",
        description: "Open a browser window for the user to log in to a job site (SEEK or LinkedIn). Saves cookies so future scrapes can access full job details including descriptions, salary, and applied status. Run this once per site before scraping.",
        parameters: z.object({
            site: z
                .string()
    ...
    server.addTool({
        ...
        description: "Check if cookies exist for job sites. Shows which sites have saved login sessions.",
        parameters: z.object({}),
        execute: async () => {
