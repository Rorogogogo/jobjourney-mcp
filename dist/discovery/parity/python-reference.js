...
export async function executePythonParityCase(parityCase, options = {}) {
    ...
    return new Promise((resolveResult, reject) => {
        ...
        child.on("close", (code) => {
            if (code !== 0) {
                reject(new Error(normalizeProcessError(stderr || stdout || `python parity bridge exited with code ${code ?? "unknown"}`)));
                return;
            }
            try {
                ...
                reject(new Error(`Unable to parse python parity output: ${error instanceof Error ? error.message : String(error)}`));
            }
        });
